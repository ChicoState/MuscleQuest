...
export class CharacterScreenComponent implements AfterViewInit {
    ...
    if (item != undefined) {
      if (this.UserData) {
        if (this.UserData.equipped[item_registry[item.id].equipment_slot!]) {
          const tempItem = this.UserData.equipped[
            item_registry[item.id].equipment_slot!
          ] as ItemState;
        ...
        this.UserData.items = this.UserData.items.filter(
          (x: ItemState) => x != item
        );
        this.userService.updateUserData(this.UserData);
  ...
  getIcon(i: number) {
    if (this.UserData)
      return this.UserData.items[i].display_icon
        ? this.UserData.items[i].display_icon
        : item_registry[this.UserData.items[i].id].icon;
  ...
  updateStats(item: ItemState | undefined) {
    ...
    if (item != undefined) {
      if (this.UserData) {
        const tempE =
          this.UserData.equipped[item_registry[item.id].equipment_slot!];
        this.currItem.name = '';
        this.currEquipped.name = '';
        item.display_name
          ? (this.currItem.name = item.display_name)
        ...
        if (tempE) {
          tempE.display_name
            ? (this.currEquipped.name = tempE.display_name)
            : (this.currEquipped.name = item_registry[tempE.id].name);
          tempE.display_name
            ? (this.currEquipped.name = tempE.display_name)
            : (this.currEquipped.name = item_registry[tempE.id].name);
        }
        ...
        if (tempE) this.currEquipped.strength = tempE.strength;

        this.currItem.dexterity = 0;
        this.currEquipped.dexterity = 0;
        this.currItem.dexterity = item.dexterity;
        if (tempE) this.currEquipped.dexterity = tempE.dexterity;
